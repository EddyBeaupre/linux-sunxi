...
#define BT_INF(...)	pr_info("[ap6210_gpio_bt]: "__VA_ARGS__)
#define BT_WRN(...)	pr_warning("[ap6210_gpio_bt]: "__VA_ARGS__)
...
static int rfkill_set_power(void *data, bool blocked)
{
	...
	case 10: /* realtek rtl8723au */
		...
		break;
        default:
		BT_WRN("no bluetooth module matched.\n" );
	}
...
static int sw_rfkill_probe(struct platform_device *pdev)
{
	...
	if (unlikely(!sw_rfkill)) {
		BT_WRN("Unable to alocate rfkill structure.\n" );
		return -ENOMEM;
		}

	ret = rfkill_register(sw_rfkill);
	if (unlikely(ret)) {
		BT_WRN("Unable to register rfkill structure.\n" );
		rfkill_destroy(sw_rfkill);
	}
	BT_INF("rfkill structure registered successfully.\n" );
	return ret;
}
...
static int sw_rfkill_remove(struct platform_device *pdev)
{
	if (likely(sw_rfkill)) {
		...
		rfkill_destroy(sw_rfkill);
		BT_INF("rfkill structure removed successfully.\n" );
	}
...
int __init sw_rfkill_init(void)
{
	...
	if (!bt_used) {
		BT_WRN("bluetooth is disable in script.fex.\n" );
		return 0;
	}

	platform_device_register(&sw_rfkill_dev);
	BT_INF("platform device registered successfully.\n" );
