            ...
            chanspec_t chspec)
{
	...
	if (unlikely(err < 0))
		pr_debug("'wl p2p_ifadd' error %d\n", err);
	else if (if_type == WL_P2P_IF_GO) {
		err = wldev_ioctl(ndev, WLC_SET_SCB_TIMEOUT, &scb_timeout, sizeof(u32), true);
		if (unlikely(err < 0))
			pr_debug("'wl scb_timeout' error %d\n", err);
	}
	return err;
}
...
wl_cfgp2p_register_ndev(struct wl_priv *wl)
{
	...
	pr_debug("%s: P2P Interface Registered\n", net->name);
