...
bcmsdh_chipmatch(uint16 vendor, uint16 device)
{
	...
	if (device == SPIH_FPGA_ID && vendor == VENDOR_BROADCOM) {
		AP6210_INFO("Found PCI SPI Host Controller\n");
		return (TRUE);
	}
...
int bcmsdh_probe(struct device *dev)
{
	...
	if  (irq < 0) {
		AP6210_INFO("%s: Host irq is not defined\n", __FUNCTION__);
		return 1;
	}
#endif 
	/* allocate SDIO Host Controller state info */
	if (!(osh = osl_attach(dev, PCI_BUS, FALSE))) {
		AP6210_INFO("%s: osl_attach failed\n", __FUNCTION__);
		goto err;
	}
	if (!(sdhc = MALLOC(osh, sizeof(bcmsdh_hc_t)))) {
		AP6210_INFO("%s: out of memory, allocated %d bytes\n",
			__FUNCTION__,
			MALLOCED(osh));
		goto err;
	}
	...
	if (!(sdh = bcmsdh_attach(osh, (void *)0,
	                          (void **)&regs, irq))) {
		AP6210_INFO("%s: bcmsdh_attach failed\n", __FUNCTION__);
		goto err;
	}
#else
	...
	if (!(sdhc->ch = drvinfo.attach((vendevid >> 16),
	                                ...
	                                (void *)regs, NULL, sdh))) {
		AP6210_INFO("%s: device attach failed\n", __FUNCTION__);
		goto err;
	}
...
bcmsdh_pci_probe(struct pci_dev *pdev, const struct pci_device_id *ent)
{
	...
	if (sd_pci_slot != 0xFFFFffff) {
		if (pdev->bus->number != (sd_pci_slot>>16) ||
			...
			AP6210_INFO("%s: %s: bus %X, slot %X, vend %X, dev %X\n",
				__FUNCTION__,
		...
		AP6210_INFO("%s: %s: bus %X, slot %X, vendor %X, device %X (good PCI location)\n",
			__FUNCTION__,
			bcmsdh_chipmatch(pdev->vendor, pdev->device)
	...
	if ((pdev->vendor == VENDOR_TI) && ((pdev->device == PCIXX21_FLASHMEDIA_ID) ||
		...
		AP6210_INFO("%s: Disabling TI FlashMedia Controller.\n", __FUNCTION__);
	...
	AP6210_INFO("%s: Found possible SDIO Host Controller: bus %d slot %d func %d irq %d\n",
		__FUNCTION__,
	...
	if (rc) {
		AP6210_INFO("%s: Cannot enable PCI device\n", __FUNCTION__);
		goto err;
	}
...
bcmsdh_register(bcmsdh_driver_t *driver)
{
...
#if defined(BCMPLATFORM_BUS)
	AP6210_INFO("Linux Kernel SDIO/MMC Driver\n");
	...
	AP6210_INFO("%s: pci_module_init failed 0x%x\n", __FUNCTION__, error);
#endif /* BCMPLATFORM_BUS */
...
static irqreturn_t wlan_oob_irq(int irq, void *dev_id)
{
	...
	if (dhdp == NULL) {
		AP6210_INFO("Out of band GPIO interrupt fired way too early\n");
		return IRQ_HANDLED;
	}
...
#endif
		if (error)
			AP6210_INFO("%s enable_irq_wake error=%d \n", __FUNCTION__, error);
